<div class="col-lg-6 col-sm-18">
    <div class="order-information">
        ...
        <ul class="list-inline order-information-list">
            ...
            <li>
                <label>{{ trans('storefront::account.view_order.order_status') }}</label>
                <span>
            ...
            <li>
                <label>{{ trans('storefront::account.view_order.payment_status') }}</label>
                <span>
                    <span class="badge {{ payment_status_badge_class($order->payment_status) }}">{{ $order->paymentStatusLabel() }}</span>
                </span>
            </li>
            ...
            <li>
                ...
                <span>
                    ...
                    @if ($order->payment_method === 'Bank Transfer')
                        <br>
                        <span style="color: #999; font-size: 13px;">{!! clean_html(setting('bank_transfer_instructions')) !!}</span>
                    @endif

                    @if ($order->paymentProof)
                        <br>
                        <a href="{{ $paymentProofUrl }}" target="_blank" rel="noopener noreferrer" style="font-size: 13px;">
            ...
            @if ($order->transaction?->transaction_id)
                <li>
                    <label>{{ trans('storefront::account.view_order.transaction_id') }}</label>
