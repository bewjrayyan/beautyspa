...
use Modules\Loyalty\Cart\CartLoyalty;
use Modules\Loyalty\Cart\NullCartLoyalty;
...
class Cart extends DarryldecodeCart implements JsonSerializable
{
    ...
    {
        return $this->getConditionsByType('loyalty')->isNotEmpty();
    }


    public function loyalty(): CartLoyalty|NullCartLoyalty
    {
        if (!$this->hasLoyalty()) {
            return new NullCartLoyalty();
        }

        return new CartLoyalty($this, $this->getConditionsByType('loyalty')->first());
    }


    public function removeLoyalty(): void
    {
        $this->removeConditionsByType('loyalty');
    }
