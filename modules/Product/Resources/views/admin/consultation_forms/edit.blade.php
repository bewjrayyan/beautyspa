...
@section('content')
    ...
    <template id="consultation-question-template">
        @include('product::admin.consultation_forms.partials.question', ['index' => '__INDEX__', 'question' => ['key' => '__KEY__', 'label' => '', 'type' => 'yes_no', 'required' => true, 'options' => '']])
    </template>
...
@push('styles')
    <style>
        ...
        @media(max-width:767px){.consultation-question-row__body-map{margin-left:0}.consultation-question-row__body-map-layout{grid-template-columns:1fr}.consultation-question-row__body-map .consultation-preview-body-map__figure img{height:250px}}
    </style>
@endpush
    ...
    <script>
        (function () {
            ...
            document.getElementById('add-consultation-question').addEventListener('click', function () {
                ...
                container.insertAdjacentHTML('beforeend', template.innerHTML.replaceAll('__INDEX__', index).replaceAll('__KEY__', key));
                refreshRows();
                container.querySelectorAll('[data-question-row]')[index].querySelector('input[type=text]').focus();
            });

            container.addEventListener('click', function (event) {
                const row = event.target.closest('[data-question-row]');
                if (!row) return;
                if (event.target.closest('[data-remove-question]')) row.remove();
                if (event.target.closest('[data-move-up]') && row.previousElementSibling) container.insertBefore(row, row.previousElementSibling);
                if (event.target.closest('[data-move-down]') && row.nextElementSibling) container.insertBefore(row.nextElementSibling, row);
                refreshRows();
            });

            container.addEventListener('change', function (event) {
                if (event.target.matches('[data-question-type]')) refreshRows();
            });
