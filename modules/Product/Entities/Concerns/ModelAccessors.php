...
trait ModelAccessors
{
    ...
    public function getFormattedPriceRangeAttribute(): ?string
    {
        ...
        $amounts = $this->variants->map(
            fn ($variant) => $variant->selling_price->convertToCurrentCurrency()->amount()
        );
        ...
        $formattedMin = Money::inDefaultCurrency($minAmount)->convertToCurrentCurrency()->format();
        $formattedMax = Money::inDefaultCurrency($maxAmount)->convertToCurrentCurrency()->format();
    ...
    public function getMediaAttribute()
    {
        return $this->files
            ...
            ->sortBy(fn ($file) => [
                $file->pivot->zone === 'base_image' ? 0 : 1,
                $file->pivot->id,
            ])
            ->values();
    }
