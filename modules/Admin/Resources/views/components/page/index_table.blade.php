@section('content')
    @if ((isset($buttons, $name) && count($buttons) > 0) || (isset($buttons) && ! is_array($buttons) && filled($buttons)))
        <div class="row">
    ...
    @php
        $tableId = isset($resource)
            ? str_replace('.', '-', $resource) . '-table'
            : '';
        $dataTableRoutePrefix = isset($resource)
            ? str_replace('.', '/', str_replace('_', '-', $resource))
            : '';
    @endphp

    <div class="box box-primary">
        <div class="box-body index-table" id="{{ $tableId }}">
            @if (isset($thead))
...
@isset($name)
    ...
    @push('scripts')
        <script type="module">
            ...
            @isset($resource)
                DataTable.set('#{{ $tableId }} .table', {
                    routePrefix: '{{ $dataTableRoutePrefix }}',
                    routes: {
                        table: 'table',
                        edit: 'edit',
                        destroy: 'destroy',
