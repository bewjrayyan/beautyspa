...
class Tab
{
    ...
    {
        $isActive = $this->active ? ' is-active' : '';
    ...
    /**
     * Render tab content for a single-page (stacked) layout without tab panes.
     *
     * @param array $data
     *
     * @return string
     */
    public function getStackedView($data = [])
    {
        return $this->getStackedPrependView() . $this->getMainView($data) . $this->getStackedAppendView();
    }


    protected function getStackedPrependView(): string
    {
        return "<section class=\"stacked-form__section\" id=\"{$this->name}\">"
            . "<h3 class=\"stacked-form__title {$this->errorClass()}\">{$this->label}</h3>"
            . '<div class="stacked-form__body">';
    }


    protected function getStackedAppendView(): string
    {
        return '</div></section>';
    }
