...
class Form
{
    ...
    public function phone($name, $title, $errors, $entity = null, $options = [])
    {
        $options['class'] = trim(array_get($options, 'class', '') . ' modern-phone-input');

        return $this->input($name, $title, $errors, $entity, array_merge($options, ['type' => 'tel']));
    }
