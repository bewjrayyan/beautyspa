...
class BlogPostController extends Controller
{
    ...
    {
        $blogPost = $this->getEntity($id);
        $blogPost->load(['tags', 'files']);

        $data = array_merge([
            'blogCategories' => $this->getBlogCategories(),
            'blogTags' => $this->getBlogTags(),
            $this->getResourceName() => $this->prepareBlogPostForForm($blogPost),
    ...
    private function prepareBlogPostForForm(BlogPost $blogPost): BlogPost
    {
        $featuredImage = $blogPost->featured_image;

        if ($featuredImage->id) {
            $blogPost->setAttribute('featured_image', [
                'id' => $featuredImage->id,
                'path' => $featuredImage->path,
            ]);
        } else {
            $blogPost->setAttribute('featured_image', new \stdClass());
        }
